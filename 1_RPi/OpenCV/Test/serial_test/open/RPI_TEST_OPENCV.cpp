...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  if(green){
    ...
    inRange( imgHSV, Scalar(0, 111, 162), Scalar(80, 255, 255), imgThreshold);
  ...
  if(orange){
    ...
    inRange(imgHSV, Scalar(0, 0 , 216), Scalar(40, 255, 255), imgThreshold);
  ...
  while(true)
	...
	tilt = atan2(bcentres[1][1] - bcentres[0][1], bcentres[0][0] - bcentres[1][0]);
      ...
      cout << "0: x: " << bcentres[0][0] << " y: " << bcentres[0][1] << endl;
      cout << "1: x: " << bcentres[1][0] << " y: " << bcentres[1][1] << endl;
 ...
 */
 ...
 data[1] = 1; //MSG length
 	...
 	if( heading > 10 )
  	...
  	if( sqrt((objects[0][0] - posX)^2 + (objects[0][1] - posY)^2) < 5){
   		cout << "Stop" << endl;
